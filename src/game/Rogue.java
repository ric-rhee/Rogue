...
public class Rogue implements Runnable {
    ...
    public Rogue(int width, int topHeight, int gameHeight, int bottomHeight) {
        displayGrid = new ObjectDisplayGrid(width, totalHeight, topHeight);
    }

    @Override
    public void run() {
        for(Displayable displayable : dungeon.getDisplayables()) {
            if(displayable.getType() == '@')
                displayGrid.setPlayerPos(displayable.getPosX(), displayable.getPosY() + dungeon.getTopHeight());
            displayable.pushObject(displayGrid, dungeon.getTopHeight());
        }
